...
def mock_secret_client():
    ...
    with patch("jcflask.config.SecretClient") as mock_client:
        ...
        mock_instance.get_secret.side_effect = lambda name: MagicMock(value=f"mock-{name}")
...
def test_missing_key_vault_name():
    ...
    with patch.dict(os.environ, {}, clear=True):
        config = ProductionConfig()
        with pytest.raises(ValueError, match="AZURE_KEY_VAULT_NAME environment variable is not set"):
